...
st.markdown("Enter student details to predict their final **Grade** based on our Random Forest analysis.")

# --- PART 1: LOAD & TRAIN (Cached for Speed) ---
@st.cache_resource
def load_and_train_model():
    # 1. Load Data
    # Ensure masked_data.csv is in the same folder!
    try:
        df = pd.read_csv("masked_data.csv")
    except FileNotFoundError:
        st.error("Error: 'masked_data.csv' not found. Please upload it to the same folder.")
        return None, None, None, None, None

    # 2. Preprocessing (Mirroring your Notebook)
    # Fill Missing
    ...
    # Using Random Forest as it was your best model
    model = RandomForestClassifier(n_estimators=200, random_state=42)
    model.fit(X, Y)
    
    # Return artifacts needed for prediction
...
if model is None:
    st.stop() # Stop if data isn't found
...
def user_input_features():
    ...
    dept = st.sidebar.selectbox("Department", ("Science", "Engineering", "Arts", "Commerce")) # Add your actual departments
    ...
    study_hours = st.sidebar.number_input("Study Hours/Week", 0, 100, 10)
    ...
    final = st.sidebar.number_input("Final Score", 0, 100, 70)
...
if st.button("Predict Grade"):
    ...
    # 2. Handle One-Hot Encoding (The Tricky Part!)
    # We turn the user input into dummies (e.g., Department_Science)
    input_df = pd.get_dummies(input_df, columns=, drop_first=True)

    # Align columns with training data: 
    # If the user selected 'Arts', but the model also expects 'Department_Science', we must add 'Department_Science' = 0
    ...
    if prediction_label in:
        ...
        st.warning("This student might need academic support.")